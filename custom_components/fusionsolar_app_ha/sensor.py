...
CHARGER_SENSORS: tuple[FusionSensorDescription, ...] = (
    # --- Status sensor is hier verwijderd ---
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    entities: list[SensorEntity] = [FusionSensor(charger, desc, is_station=False) for desc in CHARGER_SENSORS]
    if station:
        entities.extend(FusionSensor(station, desc, is_station=True) for desc in STATION_SENSORS)
...
class FusionSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator, description: FusionSensorDescription, is_station: bool) -> None:
        ...
        prefix = f"{coordinator.station_dn_id}_station" if is_station else f"{coordinator.dn_id}_charger"
        self._attr_unique_id = f"{prefix}_{description.key}"

    @property
    def device_info(self) -> DeviceInfo:
        if self._is_station:
            return DeviceInfo(identifiers={(DOMAIN, f"station_{self.coordinator.station_dn_id}")}, name=self.coordinator.station_name)
        return DeviceInfo(identifiers={(DOMAIN, f"charger_{self.coordinator.dn_id}")}, name=self.coordinator.device_name)
